...
| `multiSession`                           | Daemon-only named tasks for `sessionScope: 'user'`. The owner catalog is persisted below the workspace/channel state directory; results and interactive surfaces identify their exact source task, while webhooks, group-history backfill, loops, running-task switching, named cancellation, and per-task worktrees remain excluded. |
